...
import setUserWallet from '@/src/backend/set-user-wallet';
...
export function ConnectedWalletInfoCard() {
    ...
    async function setWalletBackendCall() {

        if(!publicKey || !session || !session.user || !session.user.token) return;

        await setUserWallet(publicKey.toString(), session.user.token)
    }
